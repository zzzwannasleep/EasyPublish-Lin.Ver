...
- [x] `Project Detail` 已进入第二轮主工作区重构：剧项目工作台、发布配置、版本快照、站点 schema 驱动字段和当前值来源提示已进入新链路
- [~] 敏感信息安全存储已开始首轮替换：`userDB` 敏感字段已切换为加密落盘兼容迁移，日志导出与原始响应诊断页已接入，Windows 出包已验证
- [~] 当前主页面重构仍未结束：`Project Detail` 虽已形成首版工作台，但 `Accounts / Projects / Dashboard / Settings` 仍未真正按新产品心智重做
- [~] 当前内核收口仍在中段：`task -> project`、renderer / service 职责边界、旧工作流页面下沉和错误返回统一还没有彻底完成

### 当前阶段判断修正（截至 2026-03-15）

到现在为止，更准确的判断应该是：

- `Phase 2` 已完成的是“应用壳、导航、布局和视觉系统”，不是“所有主页面都已经重建完成”
- `Phase 5` 已完成的是“首版可用工作流闭环”，不是“主工作区已经达到长期稳定、低耦合的最终形态”
- `Project Detail` 仍然是当前第一优先级，但焦点已经从“把旧页面挂进新壳”转向“把工作台、发布配置、版本编辑器和发布面板的职责边界继续收口”
- 目前剩余工作应继续按两条主线推进：
  - 页面主线：`Project Detail` 收口、`Accounts`、`Projects`、`Dashboard`、`Settings`
  - 内核主线：领域术语统一、存储边界继续收口、统一错误返回结构、renderer / service 边界继续收紧
- `Phase 6` 仍然重要，但它不应掩盖前台主页面和领域内核还没完全收口这个事实
...
如果后续还要继续往下拆，下一份更有价值的专题文档不再是重复一份总 backlog，而是针对当前主线单独落地，例如：

- `docs/project-detail-roadmap.md`
- `docs/renderer-boundary-cleanup.md`
